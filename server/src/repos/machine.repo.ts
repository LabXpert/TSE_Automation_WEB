...
    }
    if (machineData.last_maintenance_date !== undefined) {
      fields.push(`last_maintenance_date = $${paramIndex++}`);
      values.push(machineData.last_maintenance_date);
    }
    if (machineData.maintenance_org_id !== undefined) {
      fields.push(`maintenance_org_id = $${paramIndex++}`);
      values.push(machineData.maintenance_org_id);
    }
    if (machineData.maintenance_interval !== undefined) {
      fields.push(`maintenance_interval = $${paramIndex++}`);
      values.push(machineData.maintenance_interval);
    }
    if (fields.length === 0) {
...
        LOWER(co.org_name) LIKE LOWER($1) OR
        LOWER(mo.org_name) LIKE LOWER($1)
      ORDER BY m.id DESC
    `;
    
...
    return result.rows;
  }
    // Bakım kuruluşuna göre makineleri getir
...
    return result.rows;
  }
   // Bakımı yakında dolacak makineleri getir
...
      FROM machines m
      LEFT JOIN maintenance_orgs mo ON m.maintenance_org_id = mo.id
      WHERE m.last_maintenance_date + (m.maintenance_interval * INTERVAL '1 year') <= CURRENT_DATE + INTERVAL '${days} days'
...
      RETURNING *
    `;
    const result = await this.db.query(query, [maintenanceDate, id]);
    if (result.rows.length === 0) {
      return null;
    }
    return await this.findById(id);
...
    }
  }
    // Bakım uyarıları için özel method
...
      };
    } catch (error) {
      console.error('Database query error in getMaintenanceAlerts:', error);
      throw error;
    }
  }
}
