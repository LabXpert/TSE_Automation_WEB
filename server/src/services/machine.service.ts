...
      }
      if (!machineData.last_maintenance_date) {
        throw new Error('Son bakım tarihi gereklidir');
      }
      if (!machineData.maintenance_org_id) {
        throw new Error('Bakım kuruluşu seçiniz');
      }
      if (!machineData.maintenance_interval || machineData.maintenance_interval < 1) {
        throw new Error('Bakım aralığı en az 1 yıl olmalıdır');
      }
      // Tarih kontrolü
...
        throw new Error('Kalibrasyon aralığı en az 1 yıl olmalıdır');
      }
      if (machineData.maintenance_interval !== undefined && (machineData.maintenance_interval < 1)) {
        throw new Error('Bakım aralığı en az 1 yıl olmalıdır');
      }
      if (machineData.last_calibration_date) {
        const calibrationDate = new Date(machineData.last_calibration_date);
...
      }
      if (machineData.last_maintenance_date) {
        const maintenanceDate = new Date(machineData.last_maintenance_date);
        const today = new Date();
...
    }
  }
  async getMachinesByMaintenanceOrg(orgId: number): Promise<Machine[]> {
    try {
      return await this.machineRepo.findByMaintenanceOrg(orgId);
    } catch (error) {
      console.error('Error fetching machines by maintenance org:', error);
      throw new Error('Kuruluşa ait makineler getirilemedi');
    }
  }
...
    }
  }
  async getExpiringMaintenances(days: number = 30): Promise<Machine[]> {
    try {
      if (days < 0 || days > 365) {
        throw new Error('Gün sayısı 0-365 arasında olmalıdır');
      }
      return await this.machineRepo.findExpiringMaintenances(days);
    } catch (error) {
      console.error('Error fetching expiring maintenances:', error);
      throw new Error('Süresi dolacak bakımlar getirilemedi');
    }
  }
...
    }
  }
  async updateMaintenanceDate(id: number, maintenanceDate: string): Promise<Machine> {
    try {
      const maintenanceDateObj = new Date(maintenanceDate);
      const today = new Date();
      if (maintenanceDateObj > today) {
        throw new Error('Bakım tarihi gelecekte olamaz');
      }
      const updatedMachine = await this.machineRepo.updateMaintenanceDate(id, maintenanceDate);
      if (!updatedMachine) {
        throw new Error('Makine bulunamadı');
      }
      return updatedMachine;
...
      const [total, expiring30, expiring60, expiring90] = await Promise.all([
        this.machineRepo.findAll(),
        this.machineRepo.findExpiringMaintenances(30),
        this.machineRepo.findExpiringMaintenances(60),
        this.machineRepo.findExpiringMaintenances(90)
      ]);
      return {
        total: total.length,
        expiring30Days: expiring30.length,
